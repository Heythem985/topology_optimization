...
def run_simp(nelx=32, nely=32, volfrac=0.4, penal=3.0, E0=1.0, x_min=1e-3,
             max_iter=30, F=None, fixed_dofs=None, verbose=False):
    ...
    for it in range(max_iter):
        if verbose and (it % 5 == 0 or it == max_iter - 1):
            print(f'    run_simp: iter {it+1}/{max_iter}')
...
def make_dataset(out_dir='data/dataset', samples=10, nelx=32, nely=32, volfrac=0.4,
                 penal=3.0, max_iter=30, start_index=0, save_images=True, compress=True, seed=None):
    os.makedirs(out_dir, exist_ok=True)
    edofMat, ndof, nodenrs = build_mesh(nelx, nely)
    # make RNG reproducible per-call when seed is provided
    if seed is not None:
        np.random.seed(int(seed) + int(start_index))
    for i in range(samples):
        idx = start_index + i
        F, fixed_dofs = sample_random_bc_force(nelx, nely, ndof, nodenrs)
        print(f'Starting sample {idx} (nelx={nelx}, nely={nely}, volfrac={volfrac})', flush=True)
        rho = run_simp(nelx=nelx, nely=nely, volfrac=volfrac, penal=penal,
                       max_iter=max_iter, F=F, fixed_dofs=fixed_dofs, verbose=False)
        ...
        # choose a save index that doesn't overwrite existing files
        save_idx = idx
        npzpath = os.path.join(out_dir, f'sample_{save_idx:06d}.npz')
        while os.path.exists(npzpath):
            save_idx += 1
            npzpath = os.path.join(out_dir, f'sample_{save_idx:06d}.npz')
        if save_idx != idx:
            print(f'  Note: sample_{idx:06d} exists — using sample_{save_idx:06d} instead', flush=True)
        ...
        if save_images:
            ...
            title = f'sample_{save_idx:06d}  ({nelx}x{nely})'
            plt.title(title)
            imgpath = os.path.join(out_dir, f'sample_{save_idx:06d}.png')
        ...
        # npzpath already set above to avoid collisions (variable: npzpath)
        if compress:
        ...
        txtpath = os.path.join(out_dir, f'sample_{save_idx:06d}.txt')
        with open(txtpath, 'w', encoding='utf8') as f:
            json.dump(meta, f)

        print(f'Wrote {imgpath}, {txtpath} and {npzpath} (nelx={nelx}, nely={nely}, volfrac={volfrac}, forces={len(forces)}, fixed_nodes={len(fixed_nodes)})')
